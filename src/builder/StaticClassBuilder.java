...
public class StaticClassBuilder implements Callable<StaticClass>{
	...
	private ExpressionBuilder exprBuilder = new ExpressionBuilder();
			...
			if (line.startsWith(".method "))
			{
				...
				while (!line.equals(".end method") && index < smaliCode.size())
				{
					...
					else
					{
						...
						m.addSmaliStmt(s);
						//TODO instrumentation of method signature
						/** Instrumentation 1:
	...
	private StaticStmt buildStaticStmt(StaticMethod m, String line)
	{
		...
		exprBuilder.buildExpression(s, line);
		return s;
	}
